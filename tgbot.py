...
# =========================================================
# CONFIG: 3 БОТА В ОДНОМ ЗАПУСКЕ
# =========================================================

BOTS = [
    {
        "bot_key": "model_1",
        "token": "8339719049:AAHgD_tnsyykph8RlfItbNOOCmlUfzxtgKU",
        "admin_id": 6764552282,
    ...
    {
        "bot_key": "model_3",
        "token": "8629923443:AAH8hQJiy6iXtsS3_l9y5nYrb6vQikOE0qg",
        "admin_id": 8623099152,
        "channel_id": -1003910551079,
        "brand_name": "Skinlovers private",
        "subscriptions": {
            "vip_month": {
                "name_ru": "VIP на 1 месяц",
                "name_en": "VIP month",
...
class SubscriptionBot:
    def __init__(self, config: dict):
        ...
        self.PAYMENT_DETAILS = config["payment_details"]
    ...
    def get_user_lang(self, user_id: int) -> str:
        return self.temp_users.get(user_id, {}).get("lang", "ru")

    def format_sub_name(self, sub_key: str, lang: str) -> str:
        if sub_key in self.SUBSCRIPTIONS:
            return self.SUBSCRIPTIONS[sub_key]["name_ru"] if lang == "ru" else self.SUBSCRIPTIONS[sub_key]["name_en"]
        return sub_key

    async def is_banned(self, user_id: int) -> bool:
        async with aiosqlite.connect("db.db") as db:
            async with db.execute(
                "SELECT 1 FROM banned_users WHERE bot_key=? AND user_id=?",
                (self.bot_key, user_id)
            ) as cur:
                row = await cur.fetchone()
                return row is not None

    def lang_kb(self) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="🇷🇺 Русский", callback_data="lang_ru"),
            InlineKeyboardButton(text="🇬🇧 English", callback_data="lang_en"),
        ]])

    def user_main_kb(self, lang: str, is_admin: bool = False) -> InlineKeyboardMarkup:
        rows = [
    ...
    def subscriptions_kb(self, lang: str) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text=f"{self.SUBSCRIPTIONS['vip_month']['name_ru'] if lang == 'ru' else self.SUBSCRIPTIONS['vip_month']['name_en']} — "
                     f"{self.SUBSCRIPTIONS['vip_month']['price_ru'] if lang == 'ru' else self.SUBSCRIPTIONS['vip_month']['price_en']}",
                callback_data="sub_vip_month"
            )],
            [InlineKeyboardButton(
                text=f"{self.SUBSCRIPTIONS['vip_forever']['name_ru'] if lang == 'ru' else self.SUBSCRIPTIONS['vip_forever']['name_en']} — "
                     f"{self.SUBSCRIPTIONS['vip_forever']['price_ru'] if lang == 'ru' else self.SUBSCRIPTIONS['vip_forever']['price_en']}",
    ...
    def payment_kb(self, lang: str) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(inline_keyboard=[
    ...
    def payment_back_kb(self, lang: str) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text=TEXT[lang]["back"], callback_data="back_to_payments"),
            InlineKeyboardButton(text=TEXT[lang]["home"], callback_data="home"),
        ]])

    def admin_main_kb(self, lang: str) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(inline_keyboard=[
    ...
    def admin_back_kb(self, lang: str) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text=TEXT[lang]["back"], callback_data="admin_main"),
            InlineKeyboardButton(text=TEXT[lang]["home"], callback_data="home"),
        ]])

    async def show_main_menu(self, target, user_id: int):
        lang = self.get_user_lang(user_id)
        is_admin = user_id == self.ADMIN_ID
        text = f"{self.config['brand_name']}\n\n" + (TEXT[lang]["main_menu"] if is_admin else TEXT[lang]["user_menu"])
        kb = self.user_main_kb(lang, is_admin=is_admin)
    ...
    async def get_subscription_info(self, user_id: int):
        async with aiosqlite.connect("db.db") as db:
            async with db.execute(
                "SELECT user_id, lang, sub, expire FROM users WHERE bot_key=? AND user_id=?",
                (self.bot_key, user_id)
            ) as cur:
    ...
    def _register_handlers(self):
        ...
        async def start(message: Message):
            if await self.is_banned(message.from_user.id):
                await message.answer(TEXT["ru"]["banned_notice"])
                return

            self.admin_state["mode"] = None
            self.admin_state["target_user_id"] = None
            await message.answer(
                f"{self.config['brand_name']}\n\nChoose language / Выберите язык",
                reply_markup=self.lang_kb()
            )

        @self.dp.callback_query(F.data.startswith("lang_"))
        async def set_language(call: CallbackQuery):
            if await self.is_banned(call.from_user.id):
                await call.answer(TEXT["ru"]["banned_notice"], show_alert=True)
                return

            lang = call.data.split("_", 1)[1]
            self.temp_users.setdefault(call.from_user.id, {})
            self.temp_users[call.from_user.id]["lang"] = lang
            await self.show_main_menu(call, call.from_user.id)
        ...
        @self.dp.callback_query(F.data == "back_user_main")
        ...
        @self.dp.callback_query(F.data == "user_buy")
        async def user_buy(call: CallbackQuery):
            ...
            await call.message.edit_text(TEXT[lang]["choose_sub"], reply_markup=self.subscriptions_kb(lang))

        @self.dp.callback_query(F.data == "user_check_sub")
        async def user_check_sub(call: CallbackQuery):
            ...
            lang = self.get_user_lang(call.from_user.id)
            row = await self.get_subscription_info(call.from_user.id)
            ...
            await call.message.edit_text(
                TEXT[lang]["check_sub_ok"].format(
                    sub=self.format_sub_name(sub_key, lang),
        ...
        @self.dp.callback_query(F.data == "back_to_subs")
        ...
        @self.dp.callback_query(F.data.startswith("sub_"))
        async def choose_sub(call: CallbackQuery):
            ...
            self.temp_users.setdefault(call.from_user.id, {})
            self.temp_users[call.from_user.id]["sub"] = sub_key

            await call.message.edit_text(TEXT[lang]["choose_pay"], reply_markup=self.payment_kb(lang))

        @self.dp.callback_query(F.data == "back_to_payments")
        ...
        @self.dp.callback_query(F.data.startswith("pay_"))
        async def choose_payment(call: CallbackQuery):
            ...
            if "sub" not in self.temp_users.get(call.from_user.id, {}):
                await call.message.answer(TEXT[lang]["restart_flow"])
                return

            if method not in self.PAYMENT_DETAILS:
                await call.answer("Payment method not found", show_alert=True)
                return

            self.temp_users[call.from_user.id]["pay"] = method

            await call.message.answer(
                self.PAYMENT_DETAILS[method][lang] + "\n\n" + TEXT[lang]["send_receipt"],
                reply_markup=self.payment_back_kb(lang)
            )
        ...
        async def receipt(message: Message):
            ...
            if await self.is_banned(uid):
                return

            lang = self.get_user_lang(uid)

            if uid not in self.temp_users or "sub" not in self.temp_users[uid] or "pay" not in self.temp_users[uid]:
                await message.answer(TEXT[lang]["restart_flow"])
                return

            data = self.temp_users[uid]
            ...
            sub_name = self.format_sub_name(data["sub"], "en")
            ...
            await message.forward(self.ADMIN_ID)
            await message.answer(TEXT[lang]["receipt_sent"])

        @self.dp.callback_query(F.data.startswith("ok_"))
        async def approve(call: CallbackQuery):
            if call.from_user.id != self.ADMIN_ID:
                return

            uid = int(call.data.split("_", 1)[1])
            data = self.temp_users.get(uid)
            admin_lang = self.get_user_lang(self.ADMIN_ID)
            ...
            sub_key = data["sub"]
            days = self.SUBSCRIPTIONS[sub_key]["days"]
            ...
            async with aiosqlite.connect("db.db") as db:
                await db.execute("""
                INSERT OR REPLACE INTO users(bot_key, user_id, lang, sub, expire)
                VALUES (?, ?, ?, ?, ?)
                """, (self.bot_key, uid, data["lang"], sub_key, expire.isoformat()))
                await db.commit()

            self.temp_users.pop(uid, None)
            await call.answer("Approved")

        @self.dp.callback_query(F.data.startswith("no_"))
        async def reject(call: CallbackQuery):
            if call.from_user.id != self.ADMIN_ID:
                return

            uid = int(call.data.split("_", 1)[1])
            user_lang = self.get_user_lang(uid)
            await self.bot.send_message(uid, TEXT[user_lang]["rejected"])
            self.temp_users.pop(uid, None)
            await call.answer("Rejected")

        @self.dp.callback_query(F.data.startswith("ban_"))
        async def ban_user(call: CallbackQuery):
            ...
            async with aiosqlite.connect("db.db") as db:
                await db.execute("""
                INSERT OR REPLACE INTO banned_users(bot_key, user_id, reason, created_at)
                VALUES (?, ?, ?, ?)
                """, (self.bot_key, uid, "spam_or_fake_receipt", datetime.now().isoformat()))
                await db.execute("DELETE FROM users WHERE bot_key=? AND user_id=?", (self.bot_key, uid))
                await db.commit()

            self.temp_users.pop(uid, None)

            try:
                await self.bot.send_message(uid, TEXT[self.get_user_lang(uid)]["banned_user"])
            except Exception:
                pass

            try:
                await self.bot.ban_chat_member(self.CHANNEL_ID, uid)
                await self.bot.unban_chat_member(self.CHANNEL_ID, uid)
            except Exception:
                pass
        ...
        @self.dp.message(F.text.startswith("/unban "))
        async def unban_user(message: Message):
            ...
            async with aiosqlite.connect("db.db") as db:
                await db.execute("DELETE FROM banned_users WHERE bot_key=? AND user_id=?", (self.bot_key, uid))
                await db.commit()

            await message.answer(TEXT[lang]["unban_done"].format(uid=uid))

        @self.dp.callback_query(F.data == "admin_main")
        async def admin_main(call: CallbackQuery):
            ...
            self.admin_state["mode"] = None
            self.admin_state["target_user_id"] = None
            await call.message.edit_text(TEXT[lang]["admin_menu_title"], reply_markup=self.admin_main_kb(lang))
        ...
        async def admin_users(call: CallbackQuery):
            ...
            async with aiosqlite.connect("db.db") as db:
                async with db.execute("""
                SELECT user_id, lang, sub, expire
                FROM users
                WHERE bot_key=?
                ORDER BY expire ASC
                """, (self.bot_key,)) as cur:
            ...
            if not rows:
                await call.message.edit_text(TEXT[lang]["users_empty"], reply_markup=self.admin_back_kb(lang))
                return
            ...
            await call.message.edit_text(text, reply_markup=self.admin_back_kb(lang))

        @self.dp.callback_query(F.data == "admin_expiring")
        async def admin_expiring(call: CallbackQuery):
            ...
            if not lines:
                await call.message.edit_text(TEXT[lang]["expiring_empty"], reply_markup=self.admin_back_kb(lang))
                return

            await call.message.edit_text(
                "⏳ EXPIRING SOON\n\n" + "\n".join(lines),
                reply_markup=self.admin_back_kb(lang)
            )

        @self.dp.callback_query(F.data == "admin_banned")
        async def admin_banned(call: CallbackQuery):
            ...
            if not rows:
                await call.message.edit_text(TEXT[lang]["banned_empty"], reply_markup=self.admin_back_kb(lang))
                return
        ...
        @self.dp.callback_query(F.data == "admin_give")
        async def admin_give(call: CallbackQuery):
            if call.from_user.id != self.ADMIN_ID:
                return
            lang = self.get_user_lang(call.from_user.id)
            self.admin_state["mode"] = "give_uid"
            self.admin_state["target_user_id"] = None
            await call.message.answer(TEXT[lang]["send_user_id"], reply_markup=self.admin_back_kb(lang))
        ...
        async def admin_broadcast(call: CallbackQuery):
            if call.from_user.id != self.ADMIN_ID:
                return
            lang = self.get_user_lang(call.from_user.id)
            self.admin_state["mode"] = "broadcast"
            await call.message.answer(TEXT[lang]["broadcast_text"], reply_markup=self.admin_back_kb(lang))
        ...
        async def text_handler(message: Message):
            ...
            mode = self.admin_state.get("mode")
            lang = self.get_user_lang(message.from_user.id)

            if mode == "give_uid":
                try:
                    self.admin_state["target_user_id"] = int(message.text.strip())
                except ValueError:
                    await message.answer(TEXT[lang]["id_must_be_number"])
                    return
                self.admin_state["mode"] = "give_days"
            ...
            if mode == "give_days":
                ...
                uid = self.admin_state["target_user_id"]
                expire = datetime.now() + timedelta(days=days)
                ...
                async with aiosqlite.connect("db.db") as db:
                    ...
                    INSERT OR REPLACE INTO users(bot_key, user_id, lang, sub, expire)
                    VALUES (?, ?, ?, ?, ?)
                    """, (self.bot_key, uid, "ru", "manual", expire.isoformat()))
                    await db.commit()

                try:
                    await self.bot.send_message(uid, TEXT["ru"]["vip_granted"].format(days=days))
                except Exception:
                    pass

                self.admin_state["mode"] = None
                self.admin_state["target_user_id"] = None
                await message.answer(TEXT[lang]["done"], reply_markup=self.admin_main_kb(lang))
                return
            ...
            if mode == "broadcast":
                ...
                async with aiosqlite.connect("db.db") as db:
                    async with db.execute("SELECT user_id FROM users WHERE bot_key=?", (self.bot_key,)) as cur:
                ...
                self.admin_state["mode"] = None
                await message.answer(TEXT[lang]["broadcast_done"], reply_markup=self.admin_main_kb(lang))
                return
            ...
            async with aiosqlite.connect("db.db") as db:
                ...
                for uid, lang, sub, expire_str in rows:
                    ...
                    if now >= expire_dt:
                        ...
                        try:
                            await self.bot.send_message(uid, TEXT[lang]["expired"] + "\n" + TEXT[lang]["renew_hint"])
                        except Exception:
                            pass
                    ...
                    if 0 < time_left.total_seconds() <= 86400:
                        ...
                        if key not in warned_today:
                            try:
                                await self.bot.send_message(uid, TEXT[lang]["expiring_soon"] + "\n" + TEXT[lang]["renew_hint"])
                            except Exception:
                                pass
    ...
    async def run(self):
        ...
        logging.info("Starting bot %s (%s)", self.bot_key, me.username)
...
async def main():
    ...
    instances = [SubscriptionBot(cfg) for cfg in BOTS]
    await asyncio.gather(*[instance.run() for instance in instances])
